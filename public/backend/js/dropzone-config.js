...
if (segments[5] === 'create') {
    type = 'POST';
    url = "/media-management/albums";
    removeLink = true;
} else if (segments[6] === 'edit') {
    type = 'PUT';
    id = segments[5];
    method = segments[6];
    url = baseUrl + "/media-management/albums/" + id;
    removeLink = true;
} else {
    url = '/show';
    id = segments[5];
        ...
        if (url === '/show') {
            ...
            $("#submit").click(function (e) {
                ...
                if (myDropzone.getUploadingFiles().length === 0 && myDropzone.getQueuedFiles().length === 0) {
                    ...
                    var data;
                    if (segments[5] === 'create') {
                        data = $('#createAlbum').serializeArray();
                    } else if (segments[6] === 'edit') {
                        data = $('#editAlbum').serializeArray();
                    }
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                if (response.type == 'success') {
                                    window.location.href = "/media-management/albums";
                                } else {
                                    var errorArr = [];
            ...
            this.on("removedfile", function (file) {
                if (segments[6] === 'edit') {
                    $.post({
                        url: '/media-management/albums/photo/delete',
...
function getAlbumPhotos(myDropzone, id) 
{
    $.getJSON('/media-management/albums/' + id + '/photos', function (response) { // get the json response
        $.each(response.data, function (key, item) { // loop through it
            let path = baseUrl + "/uploads/media-management/albums/" + item.name;
